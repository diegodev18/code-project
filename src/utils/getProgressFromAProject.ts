...
export default async function (id_project: string, uuid: string): Promise<{ progress: number, percentaje: number, lenguage: string }> {
    ...
    let progress = progressTable?.find((progress: any) => progress.id_project === id_project);
    ...
    const { data } = await getGithubDirContent("diegodev18", "code-project-docs", [project.id, lenguage]);
